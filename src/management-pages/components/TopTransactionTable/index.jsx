...
const LoadinngPostsFrame = () => {
  ...
  const listItems = listProduct.map((product) => (
    <tr className="widget-lg-tr" key={product.index}>
      <td className="">
        <div className="widget-lg-img">
          {" "}
          <Skeleton circle={true} height={40} width={40} />
        </div>
      </td>
      <td className="">
        <span className="widget-lg-name">
...
const ListProducts = (props) => {
  ...
  const Button = ({ type }) => {
    ...
    else return <button className={"widget-lg-button bought"}>Unknown</button>;
  };

  const listItems = listProduct.map((product) => (
    <tr className="widget-lg-tr" key={product.id}>
      <td className="">
        {product.method === "Paypal" && (
          <img
            src="https://image.flaticon.com/icons/png/512/888/888870.png"
            alt="product img"
      ...
      <td className="td-des">
        <span className="widget-lg-description">
          {product.description || <Skeleton />}
        </span>
      </td>
      <td className="widget-lg-date">
        {handleTimeStamp(product.createdAt) || <Skeleton />}
      </td>
      ...
      <SkeletonTheme color="#e1e1f1" highlightColor="#c7c7f3">
        <table className="widget-lg-table">
          <tbody>
            ...
            {topPriceTransaction ? (
              <ListProducts listProduct={topPriceTransaction} />
            ) : (
              <LoadinngPostsFrame />
            )}
          </tbody>
